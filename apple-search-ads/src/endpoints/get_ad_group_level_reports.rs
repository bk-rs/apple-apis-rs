...
use crate::{
    ...
    objects::{
        ...
        row::AdGroupLevelRowMetaData,
    },
};

pub const URL: &str = "https://api.searchads.apple.com/api/v4/reports/campaigns/{}/adgroups";
    ...
    I: DeserializeOwned,
{
    ...
    fn render_request(&self) -> Result<Request<Body>, Self::RenderRequestError> {
        let url = Url::parse(
            format!(
                "https://api.searchads.apple.com/api/v4/reports/campaigns/{}/adgroups",
                self.campaign_id
            )
            .as_str(),
        )
...
mod tests {
    ...
    fn test_render_request() {
        let ep = GetAdGroupLevelReports::new(1, 2, Default::default(), "TOKEN");
        let req = ep.render_request().unwrap();
        assert_eq!(req.method(), "POST");
        assert_eq!(
            req.uri(),
            "https://api.searchads.apple.com/api/v4/reports/campaigns/2/adgroups"
        );
